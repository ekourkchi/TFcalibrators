...
#########################################################
def bokehTile(tileFile, jsonFile, TT=[0,0,0], DD=[2019,3,20],dynamic=False):
    ...
    for tile in citls['TILEID']:
        ...
        if tile in inci['TILEID']:
              rang=3#2  # if  IN_DESI==1 for a tile in the tile file  
        #else:  rang = 3 # default color for all tiles in the CI tile file  
        if str(tile) in data:
            rang= 3#0 #green default
...
if __name__ == "__main__":
    ...
    p = bokehTile(tileFile='ci-tiles-v7.fits', jsonFile="qa.json", TT=[0,0,0], DD=[2019,3,20],dynamic=True)
